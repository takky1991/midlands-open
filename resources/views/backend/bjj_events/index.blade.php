...
@section('container')
    ...
    <a href="{{route('bjj-events.create')}}" class="btn btn-primary">Create Event</a>
        ...
        @foreach($events as $key => $value)
            <tr>
                ...
                <td style="text-align:center;">
                    <a href="{{ URL::to('bjj-events/' . $value->id . '/edit') }}">
                        <i class="fa fa-pencil edit-icon" aria-hidden="true"></i>
                    </a>
                    <a href="{{ URL::to('bjj-events/' . $value->id) }}"
                            onclick="{{'confirmDelete' . $value->id . '()'}}">
                        <i class="fa fa-trash delete-icon" aria-hidden="true"></i>
                    </a>
					<form id="delete-{{$value->id}}" action="{{ URL::to('bjj-events/' . $value->id ) }}" method="POST" style="display: none;">
						{{ csrf_field() }}
