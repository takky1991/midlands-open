...
@section('container')
    ...
    <h1>Edit MMA Result</h1>

    <hr>

    <div class="col-md-8 col-md-offset-2">
        <form role="form" action="{{route('mma-results.update', ['mma_result' => $result->id])}}" method="POST">
            {{csrf_field()}}
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="mma_event_id" name="mma_event_id">
                            @foreach($events as $event)
                                <option value="{{$event->id}}" {{$result->mma_event_id == $event->id ? 'selected' : ''}}>{{$event->title}}</option>
                            @endforeach
                        </select>
                    </div>
