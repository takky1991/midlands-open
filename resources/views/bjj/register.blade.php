...
@section('container')
    ...
    <div class="bjj-register-page">
        @if(session()->has('error'))
            <div class="alert alert-danger" role="alert">{{session()->get('error')}}</div>
        @endif
        @if($events->isEmpty())
            <h4>Sorry, there are no upcoming events to whitch you can register to.</h4>
        @else
            ...
            <form role="form" action="{{url('/bjj/register')}}" method="POST">
                {{csrf_field()}}
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="bjj_event_id" name="bjj_event_id">
                                @foreach($events as $event)
                                    <option value="{{$event->id}}" {{old('bjj_event_id') == $event->id ? 'selected' : ''}}>{{$event->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="belt" name="belt">
                                <option value="White less than 6 months" {{old('belt') == 'White less than 6 months' ? 'selected' : ''}}>White less than 6 months</option>
                                <option value="White" {{old('belt') == 'White' ? 'selected' : ''}}>White</option>
                                <option value="Blue" {{old('belt') == 'Blue' ? 'selected' : ''}}>Blue</option>
                                <option value="Purple" {{old('belt') == 'Purple' ? 'selected' : ''}}>Purple</option>
                                <option value="Brown" {{old('belt') == 'Brown' ? 'selected' : ''}}>Brown</option>
                                <option value="Black" {{old('belt') == 'Black' ? 'selected' : ''}}>Black</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                    ...
                    <div class="checkbox col-xs-12">
                        <label>
                            <input type="checkbox" name="terms_conditions" id="terms_conditions"> I have read and understand the <a href="{{route('bjj_terms_conditions')}}" target="_blank">Terms & Conditions</a>.
                        </label>
